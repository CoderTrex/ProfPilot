    ...
    if (selectedDomain == '도메인을 선택하세요') {
      setState(() {
        showAboutDialog(
            context: context,
            applicationName: '에러.',
            children: [const Text('도메인을 선택하세요.')]);
      });
      return;
    }
    ...
    if (response.statusCode == 200) {
      if (response.body == 'success') {
        ...
        showAboutDialog(
            context: context,
            applicationName: '성공',
            children: [const Text('성공적으로 이메일을 보냈습니다.')]);
      } else if (response.body == 'already') {
        // ignore: use_build_context_synchronously
        showAboutDialog(
            context: context,
            applicationName: '이미 인증됨',
            children: [const Text('이미 인증된 이메일입니다.')]);
      } else if (response.body == 'wait') {
        // ignore: use_build_context_synchronously
        showAboutDialog(
            context: context,
            applicationName: '실패',
            children: [const Text('5분 이내로 이미 전송된 이메일이 있습니다.')]);
      } else {
        // ignore: use_build_context_synchronously
        showAboutDialog(
            context: context,
            applicationName: '실패',
            children: [const Text('이메일을 보내는데 실패했습니다.')]);
      }
      setState(() {});
    } else {
      // ignore: use_build_context_synchronously
    ...
    if (response.statusCode == 200) {
      if (response.body == 'success') {
        ...
        showAboutDialog(
            context: context,
            applicationName: '성공',
            children: [const Text('성공적으로 인증되었습니다.')]);
      } else if (response.body == 'fail') {
        // ignore: use_build_context_synchronously
        showAboutDialog(
            context: context,
            applicationName: '실패',
            children: [const Text('인증에 실패했습니다.')]);
      } else if (response.body == 'notfound') {
        // ignore: use_build_context_synchronously
        showAboutDialog(
            context: context,
            applicationName: '실패',
            children: [const Text('가입창에 작성된 이메일과 일치하는 인증코드가 없습니다.')]);
      } else {
        // ignore: use_build_context_synchronously
        showAboutDialog(
            context: context,
  ...
  Future<void> _SendSignup() async {
    ...
    if (!_passwordsMatch()) {
      ...
      showAboutDialog(
          context: context,
          applicationName: '실패',
          children: [const Text('비밀번호가 일치하지 않습니다.')]);
      return;
    }
    if (selectedDomain == '도메인을 선택하세요') {
      // ignore: use_build_context_synchronously
    ...
    if (emailController.text == '' ||
      ...
      showAboutDialog(
          context: context,
          applicationName: '실패',
          children: [const Text('모든 칸을 채워주세요.')]);
      return;
    }
    ...
    if (response.statusCode == 200) {
      if (response.body == 'success') {
        ...
        showAboutDialog(
            context: context,
            applicationName: '성공',
            children: [const Text('성공적으로 가입되었습니다.')]);
        setState(() {
          MaterialPageRoute(
              builder: (context) => breakpointName == 'MOBILE'
        ...
        showAboutDialog(
            context: context,
            applicationName: '실패',
            children: [const Text('가입에 실패했습니다.')]);
        ...
        showAboutDialog(
            context: context,
            applicationName: '실패',
            children: [const Text('이미 가입된 이메일입니다.')]);
      } else if (response.body == 'not-Verified') {
        // ignore: use_build_context_synchronously
        showAboutDialog(
            context: context,
            applicationName: '실패',
            children: [const Text('이메일 인증을 완료해주세요.')]);
      } else {
        // ignore: use_build_context_synchronously
        showAboutDialog(
            context: context,
