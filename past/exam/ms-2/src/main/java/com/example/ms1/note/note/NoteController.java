...
public class NoteController {
    ...
    public String write(@PathVariable("notebookId") Long notebookId) {
        ...
        return "redirect:/books/%d/notes/%d".formatted(notebookId, noteService.getNoteListByNotebook(notebook).get(0).getId());
    }

    @GetMapping("/{id}")
    public String detail(Model model, @PathVariable("notebookId") Long notebookId, @PathVariable("id") Long id, String keyword, String tagKeyword) {

        MainDataDto mainDataDto = mainService.getMainData(notebookId, id, keyword, tagKeyword);
